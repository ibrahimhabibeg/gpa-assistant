...
export const ProjectInfo: React.FC = () => {
  ...
  return (
    <section id="project-info-section" className="container mx-auto px-6 py-24 border-t border-white/[0.06]">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        >
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 id="team-heading" className="text-xl font-semibold text-gradient">Project Team</h3>
          </div>
          <div id="team-members-list" className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {team.map((member, i) => (
              <div
                key={i}
                id={`team-member-${i + 1}`}
        ...
        >
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 id="course-info-heading" className="text-xl font-semibold text-gradient">Course Information</h3>
          </div>
          <div id="course-details-list" className="space-y-4">
            {courseDetails.map((detail, i) => (
              <div
                key={i}
                id={`course-detail-${detail.label.toLowerCase().replace(/\s+/g, '-')}`}
                className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 border-b border-white/[0.04] pb-3 last:border-0 last:pb-0"
              >
      ...
      >
        ...
        >
          ...
          <span id="github-link-text" className="text-sm font-medium text-foreground-muted group-hover:text-foreground transition-colors">
